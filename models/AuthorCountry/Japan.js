...
var JapanData = require('../../initData/AuthorCountryData/JapanData')
...
var Japan = mongoose.model('Japan', collectionSchema); 
Japan.find({}, function (err, data) {
  // 判断数组为空
  if (!data||data.length===0) {
    JapanData.forEach(item => {
      // collection.create(item);
      Japan.create(item,function(err,docs){
        if(err) {
              console.log(err);
            } else {
              const collectionDoc = {
