...
var modernData = require('../../initData/AuthorDynastyData/modernData')
...
var modern = mongoose.model('modern', collectionSchema); 
modern.find({}, function (err, data) {
  // 判断数组为空
  if (!data||data.length===0) {
    modernData.forEach(item => {
      // collection.create(item);
      modern.create(item,function(err,docs){
        if(err) {
              console.log(err);
            } else {
              const collectionDoc = {
